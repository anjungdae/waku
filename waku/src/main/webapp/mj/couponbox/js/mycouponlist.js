...
function loadCoupon() {
	...
	$.ajax({
		async: false,
		url : "http://192.168.0.8:9999/waku/mycoupon/ByBarcode.do?uNo="+uNo,
		success : function(coupons) {
			...
			$.each(coupon, function(i, item){
				...
				temp += "<div id='dataInfo"+ i +"' >" 
	                	$.ajax({
						async: false,
	                	url : "http://192.168.0.8:9999/waku/barcode/joinGoods.do?gNo="+coupon[i].barcode.gNo,
	                	success : function(coupons) {
	            		    ...
	            		    $.ajax({
	    						async: false,
	    	                	url : "http://192.168.0.8:9999/waku/goods/joinCompany.do?cNo="+num,
	    	                	success : function(coupons) {
...
//				temp += "<div id = 'cSrial'>" +"시리얼넘버"+  coupon[i].cSerial + '</div>';
			...
			//삭제버튼-데이터 삭제 
					    //console.log("전역"+data); data전역 변수로 serial번호 받아오기. (var를 안써주면 전역으로 선언가능.)
						     ...
						     $.ajax({
									async: false,
									url : "http://192.168.0.8:9999/waku/mycoupon/mycouponDelete.do?cSerial="+coupon[i].cSerial,
							 ...
							 $.ajax({
									async: false,
				                	url : "http://192.168.0.8:9999/waku/barcode/barcodeDelete.do?cSerial="+coupon[i].barcode.cSerial,
						     ...
						     $.ajax({
										async: false,
					                	url : "http://192.168.0.8:9999/waku/goods/goodsDelete.do?gNo="+coupon[i].barcode.gNo,
									...
									$.ajax({
                                        ...
                                        console.log(goodsData);
					                		$.ajax({
											async: false,
						                	url : "http://192.168.0.8:9999/waku/company/companyDelete.do?cNo="+goodsData[0].goods.cNo,
						                	success : function(coupons) {
						            		console.log("회사테이블삭제")	
						                	}
						                	});
